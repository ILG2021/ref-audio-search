...
当前网页使用 Gradio Blocks：搜索结果显示为整行可选列表，选择结果后可在下方试听、下载或收藏。

索引操作只允许管理员在服务器命令行执行，网站不提供建库接口，也不会接受服务器目录路径。

系统不记录搜索、曝光、播放、暂停或下载行为，并显式关闭 Gradio 基础遥测。收藏是用户主动使用的功能，通过页面 JavaScript 直接保存在当前浏览器的 localStorage，不写入服务器数据库；清理浏览器站点数据后收藏会消失。上传的查询音频只用于当前检索，Gradio 临时缓存每 5 分钟清理超过 5 分钟的文件，服务重启时会完整清理。首次启动模型时，若 IndexTTS2 辅助权重缺失，上游下载模块会访问 Hugging Face 补齐权重；模型文件齐全时无需此下载。
...
反向代理部署在子路径时，通过 `--root-path` 指定 URL 前缀（例如 `/audio-search`）：

```powershell
.\.venv\Scripts\python.exe gradio_app.py --root-path /audio-search
```

为兼容已有部署，未传参数时仍会读取 `ROOT_PATH` 环境变量。
