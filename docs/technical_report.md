...
### 1.2 系统定位
本项目为面向单说话人及专业配音数据集构建的**多模态参考音频检索系统（Ref Audio Search）**。系统深度集成 **IndexTTS-2** 声学与情感表征提取能力以及 **QwenEmotion** 语义情绪对齐能力，以 Gradio 提供单进程 Python 搜索界面，并使用本地 SQLite 数据库存储索引。
...
### 4.3 本地收藏与隐私
系统不采集搜索、曝光、播放、暂停或下载行为，也不提供“相似/不相似”反馈入口，并显式关闭 Gradio 基础遥测。收藏属于用户主动操作，通过页面 JavaScript 直接保存到当前浏览器的 localStorage，不写入服务器数据库，也不建立行为历史。上传的查询音频只在 Gradio 临时缓存中参与推理，缓存每 5 分钟清理超过 5 分钟的文件，并在服务重启时完整清理。
...
本项目构建了端到端的参考音频搜索基础设施。通过将 **IndexTTS-2 的 Conditioning/EmoVec 深度特征**、Gradio 交互界面与 SQLite 索引相结合，实现零样本语音检索。

后续重点演进方向：
1. **多说话人混合检索**：引入 Speaker Diarization 与说话人聚类过滤；
2. **针对性评测与优化**：根据后续明确的问题类型建立专项评测集，再调整表征或排序策略；
